...
# Control de carril (P)
KP_STEER = 0.45          # baja si zigzaguea (0.35-0.60)
STEER_LIMIT = 400        # limite de giro en us (seguridad)
...
# === ESC arming / arranque ===
ARM_TIME = 1.0          # segundos en 1500 antes de avanzar (ajusta 0.7-2.0)
VEL_AVANCE = 1650       # tu velocidad objetivo inicial (lo que pediste)
...
# Estado de arranque ESC cuando se da "start"
start_sequence_active = False
start_time = 0.0

# Telemetría
lane_status = "IDLE"       # IDLE / TRACKING / LOST / ARMING
x_lane_center = None
x_left_edge = None
x_right_edge = None
x_img_center = None
...
if PIGPIO_OK:
    ...
    # Importante: arrancar ESC en neutro
    pi.set_servo_pulsewidth(ESC_PIN, NEUTRO)
    pi.set_servo_pulsewidth(SERVO_PIN, NEUTRO)
    time.sleep(1.2)
else:
...
def set_brake():
    global velocidad_actual, direccion_actual, start_sequence_active
    start_sequence_active = False
    velocidad_actual = NEUTRO
    direccion_actual = NEUTRO
    aplicar_pwm()
...
# ============================================================
# Prepro: CLAHE + gamma en ROI inferior
# ============================================================
clahe = cv2.createCLAHE(clipLimit=CLAHE_CLIP, tileGridSize=CLAHE_TILE)
...
# ============================================================
# Elegir par de bordes del carril
# ============================================================
def choose_lane_edges(cluster_centers):
    if len(cluster_centers) < 2:
        return None, None
    ...
    if carril_objetivo == "LEFT":
        pairs.sort(key=lambda p: (p[0], p[2]))   # más a la izq
    else:
        pairs.sort(key=lambda p: (-p[1], p[2]))  # más a la der
...
def detect_lane(img_bgr):
    ...
    clusters = sorted(cluster_x_positions(xs, dist=CLUSTER_X_DIST))
    xL, xR = choose_lane_edges(clusters)
...
# ============================================================
# Secuencia de arranque ESC: 1500 -> 1650
# ============================================================
def handle_start_sequence():
    """
    Mientras start_sequence_active esté True:
      - mantiene ESC en 1500 durante ARM_TIME
      - luego permite avanzar a VEL_AVANCE
    """
    global start_sequence_active, lane_status
    now = time.time()
    if (now - start_time) < ARM_TIME:
        lane_status = "ARMING"
        # Mantener neutro durante arming
        ramp_speed(NEUTRO)
        return False  # todavía no listo
    else:
        # Listo para avanzar
        start_sequence_active = False
        return True
...
def hilo_captura_control():
    ...
    while ejecutando:
        try:
            ...
            # Bordes seleccionados y centro carril
            if xL is not None and xR is not None:
                cv2.line(img, (xL, y0), (xL, h), (255, 0, 0), 3)
                cv2.line(img, (xR, y0), (xR, h), (0, 0, 255), 3)
                if lane_center is not None:
                    cv2.circle(img, (lane_center, int((y0 + h) / 2)), 10, (0, 255, 0), -1)
            ...
            if autonomo_activo:
                ...
                # Secuencia de arranque: 1500 por ARM_TIME
                if start_sequence_active:
                    ready = handle_start_sequence()
                    # Dirección neutra mientras arma
                    ramp_dir(NEUTRO)
                    aplicar_pwm()
                    cv2.putText(img, f"ARMING ESC... ({ARM_TIME:.1f}s)", (12, 64),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0, 255, 255), 2)
                    if not ready:
                        # aun en arming, no seguimos carril
                        pass
                    else:
                        # ya listo; continúa a tracking normal abajo
                        pass

                # Si ya no está en arming, hacemos tracking
                if not start_sequence_active:
                    if lane_center is None:
                        ...
                        # Ahora sí: avanza a 1650 (o lo que pongas)
                        ramp_speed(VEL_AVANCE)
                        ramp_dir(target_dir)
                        aplicar_pwm()
...
async def cmd_start(request):
    global autonomo_activo, start_sequence_active, start_time
    autonomo_activo = True
    # inicia secuencia ESC: 1500 por ARM_TIME, luego avanzar
    start_sequence_active = True
    start_time = time.time()
    # poner en neutro inmediato
    set_brake()
    if PIGPIO_OK:
        pi.write(LED_BLANCO, 1)
        pi.write(LED_ROJO, 0)
    return web.json_response({"ok": True, "autonomo": autonomo_activo, "arming": True})
