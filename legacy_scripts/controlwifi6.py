...
# --- Modos y Giros ---
# Cambia esta variable manualmente o mediante una señal para probar los modos
# Modos: "RECTO", "GIRO_IZQ", "GIRO_DER"
MODO_ACTUAL = "RECTO" 

ANGULO_GIRO_IZQ = 1200  # PWM para giro total izquierda
ANGULO_GIRO_DER = 1800  # PWM para giro total derecha
DELAY_INICIO_CURVA = 1.0  # Segundos de espera antes de girar el servo
...
def obtener_datos_franja(hsv_img, y, alto):
    ...
    puntos.sort()
    # Para considerar "pista detectada", necesitamos ver al menos 2 líneas (punteada y continua)
    if len(puntos) >= 2:
...
def hilo_vision():
    ...
    try:
        while ejecutando:
            ...
            # Lógica de detección: Para estar en RECTO ocupamos ver ambas líneas
            if c_inf is not None and c_sup is not None:
                ...
                cv2.putText(img_viz, "PISTA OK", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            else:
                pista_detectada = False
                cv2.putText(img_viz, "BUSCANDO LINEAS...", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        ...
        while ejecutando:
            if MODO_ACTUAL == "RECTO":
                if pista_detectada:
                    ...
                    # Si pierde la pista en modo recto, se detiene por seguridad
                    pi.set_servo_pulsewidth(ESC_PIN, NEUTRO)

            elif MODO_ACTUAL in ["GIRO_IZQ", "GIRO_DER"]:
                print(f"Iniciando secuencia de {MODO_ACTUAL}...")
                
                # 1. Delay de espera antes de girar
                time.sleep(DELAY_INICIO_CURVA)
                
                # 2. Aplicar ángulo de giro fijo
                angulo = ANGULO_GIRO_IZQ if MODO_ACTUAL == "GIRO_IZQ" else ANGULO_GIRO_DER
                pi.set_servo_pulsewidth(SERVO_PIN, angulo)
                pi.set_servo_pulsewidth(ESC_PIN, VEL_AUTONOMA)
                
                # 3. Mantener el giro HASTA que pista_detectada sea True nuevamente
                # (Significa que ya ve las dos líneas paralelas de frente)
                while not pista_detectada and ejecutando:
                    time.sleep(0.01) 
                
                print("Líneas detectadas. Volviendo a modo RECTO.")
                MODO_ACTUAL = "RECTO"
