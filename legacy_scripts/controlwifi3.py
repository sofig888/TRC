...
def hilo_vision():
    ...
    try:
        while ejecutando:
            ...
            c_inf, m_inf = obtener_centro_franja(hsv, 420)
            c_sup, m_sup = obtener_centro_franja(hsv, 320)

            # Dibujar en la imagen para el streaming
            img_viz = img.copy()
            img_viz[420:428, :] = cv2.cvtColor(m_inf, cv2.COLOR_GRAY2BGR)
            img_viz[320:328, :] = cv2.cvtColor(m_sup, cv2.COLOR_GRAY2BGR)

            if c_inf is not None and c_sup is not None:
                pista_detectada = True
                error_final = (c_inf - centro_cam) + (c_sup - c_inf) * 0.5
                cv2.line(img_viz, (c_inf, 420), (c_sup, 320), (0, 255, 0), 2)
            else:
                pista_detectada = False
