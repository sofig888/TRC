...
# ==========================================
PERSISTENCIA_LIMITE = 15 # Ajustable: frames que recuerda el stop si se pierde
DISTANCIA_SEGURA = 1.5   # Metros para velocidad máxima
DISTANCIA_PARADA = 0.3   # Metros para neutro absoluto
...
def hilo_intermitentes():
    ...
    while ejecutando:
        if intermitentes_emergencia:
            ...
            pi.write(LED_GIRO_IZQ, not pi.read(LED_GIRO_IZQ)); pi.write(LED_GIRO_DER, 0)
        elif direccion_actual > NEUTRO:
            pi.write(LED_GIRO_DER, not pi.read(LED_GIRO_DER)); pi.write(LED_GIRO_IZQ, 0)
        else:
            pi.write(LED_GIRO_IZQ, 0); pi.write(LED_GIRO_DER, 0)
        time.sleep(0.4)
...
def main():
    ...
    # Selección de cámara (Index 4)
    ctx = rs.context()
    dispositivos = ctx.query_devices()
    if len(dispositivos) < 1: return
    
    pipeline = rs.pipeline()
    config = rs.config()
    # Si tienes varias cámaras, aquí podrías forzar el serial del index 4
    # config.enable_device(dispositivos[3].get_info(rs.camera_info.serial_number))
